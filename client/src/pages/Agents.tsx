...
import { PositionSelect } from "@/components/PositionManager";
...
export default function Agents() {
  ...
  const [editAgent, setEditAgent] = useState<{
    id: number; name: string; phone: string; note: string; isActive: boolean; linkedUserId: number | ""; positionId: number | null;
  } | null>(null);
  ...
  return <div className="mx-auto w-full max-w-[1500px]">
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="rounded-2xl sm:max-w-md"><DialogHeader><DialogTitle>Yangi agent qo‘shish</DialogTitle><DialogDescription>Agentning asosiy aloqa ma’lumotlarini kiriting.</DialogDescription></DialogHeader><div className="space-y-4 py-2"><div className="space-y-2"><Label>Agent F.I.Sh.</Label><Input className="finance-input" value={name} onChange={event => setName(event.target.value)} placeholder="Masalan: Akmal Karimov" /></div><div className="space-y-2"><Label>Telefon raqami</Label><Input className="finance-input" value={phone} onChange={event => setPhone(event.target.value)} placeholder="+998 90 000 00 00" /></div><div className="space-y-2"><Label>Lavozim</Label><PositionSelect value={positionId} onChange={setPositionId} /></div><div className="space-y-2"><Label>Izoh</Label><Textarea value={note} onChange={event => setNote(event.target.value)} placeholder="Qo‘shimcha ma’lumot..." className="min-h-24 rounded-xl" /></div></div><DialogFooter><Button variant="outline" onClick={() => setOpen(false)}>Bekor qilish</Button><Button disabled={name.trim().length < 2 || create.isPending} onClick={() => create.mutate({ name, phone: phone || undefined, note: note || undefined, positionId })}>{create.isPending ? "Saqlanmoqda..." : "Agentni saqlash"}</Button></DialogFooter></DialogContent></Dialog>
      ...
      <DialogContent className="rounded-2xl sm:max-w-md">
        ...
        {editAgent && (
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-2"><Label>Lavozim</Label><PositionSelect value={editAgent.positionId} onChange={next => setEditAgent({ ...editAgent, positionId: next })} /></div>
            ...
            onClick={async () => {
              ...
              try {
                await updateAgent.mutateAsync({
                  ...
                  positionId: editAgent.positionId,
                });
