    ...
    <div className="mx-auto w-full max-w-[1200px]">
      ...
      <SectionCard title="Mijozni tanlang" description="Kod, nom yoki agent bo‘yicha qidiring">
        <div className="mb-4 grid gap-3 sm:grid-cols-[1fr_220px]">
          ...
          <select value={agentId} onChange={event => setAgentId(event.target.value)} className="finance-input border px-3 text-muted-foreground">
        ...
        {clients.isLoading ? (
          <p className="py-8 text-center text-xs text-muted-foreground">Yuklanmoqda...</p>
        ) : items.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {items.map(client => (
              <button
                ...
                className="flex w-full items-center gap-3 rounded-xl border border-transparent bg-muted/70 p-3 text-left transition-colors hover:border-primary/30 hover:bg-primary/5"
              >
                <div className="grid size-9 shrink-0 place-items-center rounded-xl bg-card text-primary shadow-sm"><FileText className="size-4" /></div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-bold text-foreground">{client.name}</p>
                  <p className="mt-0.5 truncate text-xs text-muted-foreground">{client.code} • {client.agentName || "agentsiz"}</p>
                </div>
