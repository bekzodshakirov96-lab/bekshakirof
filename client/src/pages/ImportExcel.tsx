  ...
  return <div className="mx-auto w-full max-w-[1350px]">
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
      <SectionCard title="Excel faylini yuklash" description="Qarzdorlik_Hisoboti.xlsm shabloni va uning yangilangan nusxalari qo‘llab-quvvatlanadi">
        ...
        <button type="button" onClick={() => inputRef.current?.click()} onDragOver={event => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={event => { event.preventDefault(); setDragging(false); validateAndSelect(event.dataTransfer.files?.[0]); }} className={`group flex min-h-64 w-full flex-col items-center justify-center rounded-3xl border-2 border-dashed px-6 text-center transition ${dragging ? "border-primary bg-cyan-50" : "border-border bg-muted/70 hover:border-cyan-300 hover:bg-cyan-50/40"}`}>
          <div className="grid h-16 w-16 place-items-center rounded-2xl bg-card text-primary shadow-sm ring-1 ring-slate-100 transition group-hover:-translate-y-1"><FileSpreadsheet className="h-7 w-7" /></div>
          <p className="mt-5 text-sm font-bold text-foreground">Faylni shu yerga tashlang yoki tanlash uchun bosing</p>
          <p className="mt-2 text-xs leading-5 text-muted-foreground">.XLSX, .XLSM yoki .XLS • maksimal hajm 30 MB</p>
        </button>
        {selected ? <div className="mt-4 flex flex-col gap-4 rounded-2xl border border-cyan-100 bg-cyan-50/60 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-card text-emerald-600 shadow-sm"><FileCheck2 className="h-5 w-5" /></div><div><p className="text-sm font-bold text-foreground">{selected.name}</p><p className="text-xs text-muted-foreground">{(selected.size / 1024 / 1024).toFixed(2)} MB</p></div></div><Button disabled={busy} onClick={handleUpload} className="rounded-xl bg-primary text-xs font-semibold"><FileUp className="mr-2 h-4 w-4" />{busy ? "Import qilinmoqda..." : "Importni boshlash"}</Button></div> : null}
        {busy ? <div className="mt-4"><div className="mb-2 flex items-center justify-between text-xs font-medium text-muted-foreground"><span>Fayl tekshirilmoqda va ma’lumotlar yangilanmoqda</span><span>Iltimos, kuting</span></div><Progress value={reading ? 25 : 70} className="h-2" /></div> : null}
      </SectionCard>
