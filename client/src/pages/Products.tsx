...
import { CircleDollarSign, PackageCheck, Pencil, Plus, Search } from "lucide-react";
...
const emptyDraft: ProductDraft = { code: "", name: "", unit: "dona", price: "0", containerType: "", containerUnitsPerItem: "1" };
...
export default function Products() {
  ...
  const averagePrice = rows.length ? rows.reduce((sum, row) => sum + row.price, 0) / rows.length : 0;
  const isSaving = updatePrice.isPending || updateContainer.isPending || create.isPending;
  ...
  async function submit() {
    ...
    try {
      ...
      if (draft.id) await updatePrice.mutateAsync({ id: draft.id, price });
      else {
