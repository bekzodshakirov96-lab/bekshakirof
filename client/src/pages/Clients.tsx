...
export default function Clients() {
  ...
  const [search, setSearch] = useState(""); const [agentId, setAgentId] = useState(""); const [debtOnly, setDebtOnly] = useState(false); const [page, setPage] = useState(1); const [open, setOpen] = useState(false);
  const [form, setForm] = useState({ code: "", name: "", agentId: "", phone: "", address: "", openingDebt: "" });
  const [statementClient, setStatementClient] = useState<{ id: number; name: string } | null>(null);
  const [editClient, setEditClient] = useState<{
    id: number; code: string; name: string; agentId: string; phone: string; address: string; openingDebt: string; isActive: boolean;
  } | null>(null);
  ...
  const clients = trpc.clients.list.useQuery({ search: search || undefined, agentId: agentId ? Number(agentId) : undefined, debtOnly, page, pageSize: 25 });
  const create = trpc.clients.create.useMutation({ onSuccess: async () => { toast.success("Mijoz qo‘shildi"); setOpen(false); setForm({ code: "", name: "", agentId: "", phone: "", address: "", openingDebt: "" }); await utils.clients.list.invalidate(); }, onError: error => toast.error(error.message) });
  ...
  return <div className="mx-auto w-full max-w-[1600px]">
    ...
    <SectionCard title="Mijozlar ro‘yxati" description="Qidiruv, agent va qarzdorlik bo‘yicha filter">
      <div className="mb-4 grid gap-3 lg:grid-cols-[minmax(260px,1fr)_220px_170px]">
