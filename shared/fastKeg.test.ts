...
describe("Tezkor KEG jonli hisoblari", () => {
  it("KEG 30/50 savdosi, qaytgan tara, kassa va yakuniy qarzni bir qatorda hisoblaydi", () => {
    const result = calculateFastKegRow(
      { keg30: 2, keg50: 3, returned30: 1, returned50: 2, cash: 400_000, terminal: 0, click: 0, transfer: 0 },
  ...
  it("KEG 30 va KEG 50 tara qoldiqlarini o‘zaro aralashtirmaydi", () => {
    const result = calculateFastKegRow(
      { keg30: 0, keg50: 1, returned30: 2, returned50: 0, cash: 0, terminal: 0, click: 0, transfer: 0 },
  ...
  it("faqat kassa kiritilganda savdo yoki tara yaratmasdan qarzni kamaytiradi", () => {
    const result = calculateFastKegRow(
      { keg30: 0, keg50: 0, returned30: 0, returned50: 0, cash: 250_000, terminal: 0, click: 0, transfer: 0 },
  ...
  it("mahsulot tara koeffitsiyentini berilgan tara soniga qo‘llaydi", () => {
    const result = calculateFastKegRow(
      { keg30: 3, keg50: 0, returned30: 1, returned50: 0, cash: 0, terminal: 0, click: 0, transfer: 0 },
  ...
  it("naqd, terminal va perechisleniya uchtasi birga qarzni kamaytiradi", () => {
    const result = calculateFastKegRow(
      { keg30: 0, keg50: 0, returned30: 0, returned50: 0, cash: 100_000, terminal: 50_000, click: 0, transfer: 30_000 },
  ...
  it("Click va Перечисление alohida to'lov turlari sifatida ikkalasi ham qarzni kamaytiradi", () => {
    const result = calculateFastKegRow(
      { keg30: 0, keg50: 0, returned30: 0, returned50: 0, cash: 0, terminal: 0, click: 40_000, transfer: 60_000 },
      { currentDebt: 500_000, currentKeg30Balance: 0, currentKeg50Balance: 0 },
      { keg30Price: 0, keg50Price: 0, keg30UnitsPerItem: 1, keg50UnitsPerItem: 1 },
    );

    expect(result.endingDebt).toBe(400_000);
  });

  it("bir nechta tanlangan mijozning KEG, tara, to‘lov turlari, savdo va qarzini jamlaydi", () => {
    const summary = summarizeFastKegRows([
      { keg30: 2, keg50: 1, returned30: 1, returned50: 0, cash: 100, terminal: 40, click: 5, transfer: 10, saleAmount: 500, endingDebt: 900 },
      { keg30: 0, keg50: 3, returned30: 0, returned50: 2, cash: 200, terminal: 0, click: 15, transfer: 60, saleAmount: 700, endingDebt: 1_200 },
    ]);
